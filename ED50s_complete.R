...
input<-read.table("Input_data/VTK2022_PAM.txt", header = T, sep = "\t") 
...
input$Colony=as.factor(paste(input$Strain,input$Treatment,input$Replicate, sep = "_")) # replicate per treatment per strain
levels(input$Colony)
...
#check for normality 
hist(ed50_df$ED50) #Gaussian shape?
shapiro.test(ed50_df$ED50) #normality?
...
H2_ED50s<-ed50_df[ed50_df$Strain=="H2", ]
FOO3_ED50s<-ed50_df[ed50_df$Strain=="FOO3", ]

pairwise.t.test(H2_ED50s$transformed_ED50s,H2_ED50s$Treatment, p.adj = "fdr")
pairwise.t.test(FOO3_ED50s$transformed_ED50s,FOO3_ED50s$Treatment, p.adj = "fdr")
